...
pacman -Syu --needed efibootmgr grub networkmanager pipewire-pulse pipewire-jack vulkan-radeon libappimage rsync rclone git git-lfs trash-cli 7zip dosfstools gstreamer gst-plugins-base gst-plugins-good gst-plugins-bad gst-plugins-ugly xdg-desktop-portal-hyprland xdg-desktop-portal-gtk wayland flatpak gammastep slurp grim fzf chafa ollama wl-clipboard openssh ufw fastfetch docker docker-compose hyprpicker hyprland waybar gnome-disk-utility gnome-system-monitor alacritty rofi pavucontrol krita chromium neovim gcc mingw-w64-gcc clang zig zls rust rust-analyzer shfmt npm nodejs lua-language-server stylua adw-gtk-theme breeze5 noto-fonts noto-fonts-cjk noto-fonts-emoji ttf-jetbrains-mono ttf-jetbrains-mono-nerd papirus-icon-theme
...
cp -r /softwares/user.sh ${USERHOME}
cp -r /softwares/boot.sh ${USERHOME}
cp -r /softwares/alacritty.sh ${USERHOME}
cp -r /softwares/chroot.sh ${USERHOME}

sudo -u ${USERNAME} ${USERHOME}/user.sh ${USERHOME}
