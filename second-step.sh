...
pacman -Syu --needed efibootmgr grub networkmanager pipewire-pulse jack2 vulkan-radeon lib32-vulkan-radeon wine wine-mono libappimage unzip gstreamer gst-plugins-base gst-plugins-good gst-plugins-bad gst-plugins-ugly xdg-desktop-portal-hyprland xdg-desktop-portal-gtk wayland flatpak gammastep slurp grim fzf chafa ollama wl-clipboard openssh ufw fastfetch docker docker-compose hyprpicker hyprland waybar gnome-disk-utility gnome-system-monitor alacritty rofi pavucontrol krita chromium mangohud neovim git gcc mingw-w64-gcc clang zig zls rust rust-analyzer shfmt npm nodejs lua-language-server stylua papirus-icon-theme adw-gtk-theme breeze5 ttf-jetbrains-mono ttf-jetbrains-mono-nerd noto-fonts noto-fonts-cjk noto-fonts-emoji
...
tee /usr/bin/fopen >/dev/null <<'EOF'
#!/bin/bash
dir=$(find ~/ -type d | fzf)
...
#!/bin/bash
copy=$(find ~/ -type f | fzf)
paste=$(find ~/ -type d | fzf)

cp -r ${copy} ${paste}
EOF
tee /usr/bin/fedit >/dev/null <<'EOF'
#!/bin/bash
dir=$(find ~/ -type d | fzf)
...
chmod +x /usr/bin/fopen
chmod +x /usr/bin/fcopy
chmod +x /usr/bin/fedit
