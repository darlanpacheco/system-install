...
pacman -Syu --needed efibootmgr grub networkmanager pipewire-pulse pipewire-jack vulkan-radeon libappimage trash-cli 7zip gstreamer gst-plugins-base gst-plugins-good gst-plugins-bad gst-plugins-ugly xdg-desktop-portal-hyprland xdg-desktop-portal-gtk wayland flatpak gammastep slurp grim fzf chafa ollama wl-clipboard openssh ufw fastfetch docker docker-compose hyprpicker hyprland waybar gnome-disk-utility gnome-system-monitor alacritty rofi pavucontrol krita chromium neovim rclone git git-lfs gcc mingw-w64-gcc clang zig zls rust rust-analyzer shfmt npm nodejs lua-language-server stylua papirus-icon-theme adw-gtk-theme breeze5 ttf-jetbrains-mono ttf-jetbrains-mono-nerd noto-fonts noto-fonts-cjk noto-fonts-emoji
...
mkdir -p ${USERHOME}/Desktop ${USERHOME}/Documents ${USERHOME}/Downloads ${USERHOME}/Music ${USERHOME}/Videos ${USERHOME}/Synchronization
...
tee /usr/bin/ssend >/dev/null <<'EOF'
#!/bin/bash
rclone check Science: ~/Synchronization/ --exclude "Personal Vault/**" --progress
EOF
tee /usr/bin/ssend >/dev/null <<'EOF'
#!/bin/bash
rclone sync Science: ~/Synchronization/ --exclude "Personal Vault/**" --progress
EOF
tee /usr/bin/ssend >/dev/null <<'EOF'
#!/bin/bash
rclone sync ~/Synchronization/ Science: --exclude "Personal Vault/**" --progress
EOF
