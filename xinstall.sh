...
xbps-install xz util-linux dosfstools btrfs-progs curl
...
curl -L "${URL}" -o /tmp/void-rootfs.tar.xz
tar xf /tmp/void-rootfs.tar.xz -C /mnt

mkdir -p /mnt/var/db/xbps/keys
cp -r /var/db/xbps/keys/* /mnt/var/db/xbps/keys/

xbps-install -Sy -r /mnt base-system linux-lts linux-firmware grub NetworkManager
...
xchroot /mnt /softwares/chroot.sh
